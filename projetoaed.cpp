        ...
        if (x.StudentName.compare(nomeEstudante)==0){
            ...
            for (auto y:arr2){
                ...
                if(y.Weekday.compare("Monday")==0) {
                    v1.push_back(y);
                    sort(v1.begin()->StartHour, v1.end()->StartHour, intcomp);
                    for (const auto &n: v1) {
                ...
                if(y.Weekday.compare("Tuesday")==0){
                    v2.push_back(y);
                    sort(v2.begin()->StartHour, v2.end()->StartHour, intcomp);
                    for (const auto &n: v2) {
                        cout << nomeEstudante << " has a " << n.Type << " class for UC " << n.UcCode << " from "
                             << n.StartHour << " to " << n.StartHour + n.Duration << " on " << n.Weekday << endl;
                    }
                ...
                if(y.Weekday.compare("Wednesday")==0){
                    v3.push_back(y);
                    sort(v3.begin()->StartHour, v3.end()->StartHour, intcomp);
                    for (const auto &n: v3) {
                        cout << nomeEstudante << " has a " << n.Type << " class for UC " << n.UcCode << " from "
                             << n.StartHour << " to " << n.StartHour + n.Duration << " on " << n.Weekday << endl;
                    }
                ...
                if(y.Weekday.compare("Thursday")==0){
                    v4.push_back(y);
                    sort(v4.begin()->StartHour, v4.end()->StartHour, intcomp);
                    for (const auto &n: v4) {
                        cout << nomeEstudante << " has a " << n.Type << " class for UC " << n.UcCode << " from "
                             << n.StartHour << " to " << n.StartHour + n.Duration << " on " << n.Weekday << endl;
                    }
                ...
                if(y.Weekday.compare("Friday")==0){
                    v5.push_back(y);
                    sort(v5.begin()->StartHour, v5.end()->StartHour, intcomp);
                    for (const auto &n: v5) {
                        cout << nomeEstudante << " has a " << n.Type << " class for UC " << n.UcCode << " from "
                             << n.StartHour << " to " << n.StartHour + n.Duration << " on " << n.Weekday << endl;
                    }
...
int main() {
    ...
    myFile.open("/Users/goncalocosta/Desktop/2ano/projetoaed/schedule/classes.csv");
    ...
    myFile.open("/Users/goncalocosta/Desktop/2ano/projetoaed/schedule/classes_per_uc.csv");
    getline(myFile,CurrentLine);
    while(getline(myFile,CurrentLine)) {
        stringstream inputString(CurrentLine);
    ...
    myFile.open("/Users/goncalocosta/Desktop/2ano/projetoaed/schedule/students_classes.csv");
    getline(myFile,CurrentLine);
